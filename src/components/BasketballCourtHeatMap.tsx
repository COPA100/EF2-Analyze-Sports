...
const ZONE_POLYGONS: Record<string, { x: number; y: number }[]> = {
  ...
  '2': [P.arcL_start, P.paintTL, P.paintBL, P.paintBMid, P.arcMid, P.arcL_5, P.arcL_4, P.arcL_int, P.arcL_3, P.arcL_2, P.arcL_1, P.arcL_endStr],
  '3': [P.paintTR, P.arcR_start, P.arcR_endStr, P.arcR_1, P.arcR_2, P.arcR_3, P.arcR_int, P.arcR_4, P.arcR_5, P.arcMid, P.paintBMid, P.paintBR],
  '4': [P.tl, P.arcL_start, P.arcL_endStr, P.arcL_1, P.arcL_2, P.arcL_3, P.arcL_int, P.diagL_bot, P.bl],
  '5': [P.arcL_int, P.arcL_4, P.arcL_5, P.arcMid, P.arcR_5, P.arcR_4, P.arcR_int, P.diagR_bot, P.diagL_bot],
  '6': [P.arcR_start, P.tr, P.br, P.diagR_bot, P.arcR_int, P.arcR_3, P.arcR_2, P.arcR_1, P.arcR_endStr],
};
...
export default function BasketballCourtHeatMap({
  ...
  const emptyStateSubtextClasses = compact ? 'text-sm mt-2 opacity-90' : 'text-lg mt-3 opacity-90';
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className={legendWrapperClasses}>
        <div className={`${compact ? 'text-xs' : 'text-sm'} font-bold text-gray-300`}>Miss-Heavy</div>
        <div
          className={gradientClasses}
          style={{ background: 'linear-gradient(90deg, #ef4444 0%, #facc15 50%, #22c55e 100%)' }}
        />
        <div className={`${compact ? 'text-xs' : 'text-sm'} font-bold text-gray-300`}>Make-Heavy</div>
      </div>
          ...
          {zoneStats.map((stat) => (
            <div
              ...
              style={stat.total === 0 ? {
                ...
                backgroundColor: getHeatmapColor(stat.percentage / 100, 0.9),
                borderColor: getHeatmapColor(stat.percentage / 100, 1),
                color: getZoneCardTextColor(stat.percentage, stat.total),
              }}
            >
              <div className={`${compact ? 'text-base' : 'text-xl'} font-bold mb-1`}>Zone {stat.zone}</div>
              <div className={`${compact ? 'text-xs' : 'text-sm'} font-medium mb-2 opacity-90`}>{stat.label}</div>
              <div className={`${compact ? 'text-2xl' : 'text-4xl'} font-extrabold mb-1`}>{stat.percentage}%</div>
      ...
      {showQuickInsight && shots.length > 0 && (
        <div className={insightClasses}>
          <div className={`${compact ? 'text-base' : 'text-lg'} font-bold text-white mb-2`}>Quick Insight:</div>
          <div className={`${compact ? 'text-sm' : 'text-base'} text-gray-200 leading-relaxed`}>
            {(() => {
              const zonesWithData = zoneStats.filter((z) => z.total > 0);
              if (zonesWithData.length === 0) return 'No data yet.';
              const best = zonesWithData.reduce((a, b) => (a.percentage > b.percentage ? a : b));
              const worst = zonesWithData.reduce((a, b) => (a.percentage < b.percentage ? a : b));
              return (
                <>
                  Strongest from <strong>Zone {best.zone} ({best.label})</strong> at {best.percentage}%.
                  {best.zone !== worst.zone && (
                    <>
                      {' '}
                      Practice more on <strong>Zone {worst.zone} ({worst.label})</strong> ({worst.percentage}%).
                    </>
                  )}
                </>
              );
